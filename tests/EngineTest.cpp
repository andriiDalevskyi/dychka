...
// The converted Gross Beat banks (banks/GrossBeat, generated by tools/gb2dychka.py) must load and
// keep their geometry: run from the repository root; skipped when the folder is missing.
static void testGrossBeatBanks()
{
    auto dir = juce::File::getCurrentWorkingDirectory().getChildFile ("banks/GrossBeat");
    if (! dir.isDirectory())
    {
        std::printf ("  (banks/GrossBeat not found - Gross Beat bank test skipped)\n");
        return;
    }
    int files = 0;
    for (const auto& f : dir.findChildFiles (juce::File::findFiles, false, "*.dychka-bank.json"))
    {
        Bank b = factoryBank();
        const int n = BankStore::loadBank (f, b) ? 1 : 0;
        CHECK (n == 1, (juce::String ("Gross Beat bank loads: ") + f.getFileName()).toRawUTF8());
        ++files;
        for (int k = 0; k < kNumKinds; ++k)
            for (const auto& e : b.of ((EnvKind) k))
            {
                CHECK (! e.points.empty() && e.points.front().x == 0.0f && (int) e.points.size() <= kMaxPoints, "Gross Beat slot is well formed");
                for (size_t i = 1; i < e.points.size(); ++i) CHECK (e.points[i].x >= e.points[i - 1].x, "Gross Beat points sorted");
            }
        if (f.getFileName().startsWith ("GB Patterns"))
        {
            const auto& basic1 = b.of (EnvKind::time)[1];
            CHECK (basic1.name == "Basic 1" && basic1.lengthBeats == 4, "Patterns slot 2 is Basic 1 over one bar");
            CHECK (std::abs (basic1.valueAt (0.40f) - 0.0625f) < 1.0e-5f && basic1.points[1].hold, "Basic 1 stutter steps back a sixteenth per sixteenth");
            const auto& gate = b.of (EnvKind::volume)[1];
            CHECK (gate.name == "2 Beat Gate" && gate.valueAt (0.1f) == 1.0f && gate.valueAt (0.3f) == 0.0f, "2 Beat Gate converted as hold steps");
        }
        if (f.getFileName().startsWith ("GB Momentary"))
        {
            const auto& rev = b.of (EnvKind::time)[20];
            CHECK (rev.name == "Reverse" && rev.retrigger && std::abs (rev.valueAt (0.5f) - 0.5f) < 1.0e-5f, "Reverse: live to two bars back over one bar, RETRIG");
            const auto& rep = b.of (EnvKind::time)[9];
            CHECK (rep.name == "1/8 Repeat" && rep.points.size() == 33 && std::abs (rep.valueAt (0.5f) - 16.0f / 8.0f / 8.0f) < 1.0e-5f, "1/8 Repeat = 32 exact steps of an eighth");
        }
    }
    CHECK (files == 8, "eight Gross Beat banks");
}
